...
- **상태별로 다르게:** `coding.png`, `done.png`, `committing.png`, `pushing.png`, `testing.png`, `rejected.gif`, `thanks.png`, `annoyed.png`, `waiting.png`, `thinking.png`, `idle.png`, `error.png` …
- **한 장으로 통일:** `character.png` (또는 `.gif`/`.jpg`) — 상태별 파일이 없으면 이걸 사용

### 우선순위

```
<상태>.gif → <상태>.png → <상태>.jpg/.jpeg → character.gif → character.png → character.jpg → (없으면) 이모지
```
